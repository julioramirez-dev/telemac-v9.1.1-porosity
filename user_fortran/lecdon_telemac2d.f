...
!! @param[in]     CAS_FILE
!! @param[in]     DICO_FILE
!! @param[in]     CAS_FILE_GAIA
!! @param[in]     DICO_FILE_GAIA
...
!-----------------------------------------------------------------------
!
      INTRINSIC MAX,INT,MOD,ACOS,LOG,TAN,ATAN
!
!-----------------------------------------------------------------------
!
      WRITE(LU,*) '=========================================='
      WRITE(LU,*) 'MI LECDON_TELEMAC2D ESTA FUNCIONANDO'
      WRITE(LU,*) '=========================================='
...
!
      DO I=1,MAXLU_T2D
        ...
        ELSEIF(T2D_FILES(I)%TELNAME.EQ.'T2DBB2') THEN
          T2DBB2 = I
        ELSEIF(T2D_FILES(I)%TELNAME.EQ.'T2DOBS') THEN
          T2DOBS = I
        ELSEIF(T2D_FILES(I)%TELNAME.EQ.'T2DOUT') THEN
          T2DOUT = I
        ELSEIF(T2D_FILES(I)%TELNAME.EQ.'T2DOGE') THEN
          T2DOGE = I
        ELSEIF(T2D_FILES(I)%TELNAME.EQ.'T2DOTS') THEN
          T2DOTS = I
        ELSEIF(T2D_FILES(I)%TELNAME.EQ.'T2DOHY') THEN
          T2DOHY = I
...
!
      ...
      ALLOCATE(OK_U(MAXFRO))
      ALLOCATE(OK_V(MAXFRO))
...
!
      ...
      IF(NSOFT.NE.0) THEN
        ...
        END DO
      ENDIF
      LIQTYP = MOTINT( ADRESS(1,124) )
      IF(ANY(OPT_SOFT.EQ.3).AND.LIQTYP.NE.2) THEN
        WRITE(LU,*) 'OPTION FOR SOFT BOUNDARIES = 3 REQUIRES ' //
     &              'OPTION FOR LIQUID BOUNDARIES FILE = 2'
        CALL PLANTE(1)
        STOP
      ENDIF
...
!     COEFFICIENT OF WIND INFLUENCE FORMULA
      FAIROPT = MOTINT(ADRESS(1,125))
...
!     ATMOSPHERIC PRESSURE UNIT
      PRESSURE_UNIT = MOTINT(ADRESS(1,61))
!
...
!
      IF(NTRAC.GT.0) THEN
        IF(TROUVE_KEY(2,10).GE.1.AND.DIMEN(2,10).GE.NTRAC) THEN
          DO I=1,NTRAC
...
!         COMPLETING WITH THE LAST GIVEN
        ...
        ENDIF
!
        IF(TROUVE_KEY(1,46).GE.1.AND.DIMEN(1,46).GE.NTRAC) THEN
          DO I=1,NTRAC
            MVIST(I) = MOTINT(ADRESS(1,46)+I-1)
          ENDDO
        ELSEIF(TROUVE_KEY(1,46).GE.1.AND.
     &         DIMEN(1,46).LT.NTRAC.AND.DIMEN(1,46).GT.0) THEN
!         READING WHAT HAS BEEN GIVEN
          DO I=1,DIMEN(1,46)
            MVIST(I) = MOTINT(ADRESS(1,46)+I-1)
          ENDDO
!         COMPLETING WITH THE LAST GIVEN
          DO I=DIMEN(1,46)+1,NTRAC
            MVIST(I) = MOTINT(ADRESS(1,46)+DIMEN(1,46)-1)
          ENDDO
        ENDIF
      ENDIF
...
!     SOURCES ARE GIVEN BY COORDINATES
        ...
        IF(PROTYP.EQ.3) THEN
          ...
          IF(NREJEX.EQ.NREJET.AND.NREJEY.EQ.NREJET) THEN
            DO K=1,NREJET
              XSCE(K)=R*(XSCE(K)*PS4/45.D0-LONGIRAD)
              TAN1=TAN(YSCE(K)*PS4/90.D0+PS4)
              YSCE(K)=R*(LOG(TAN1)-LOG(TAN2))
            ENDDO
          ENDIF
        ENDIF
...
!     THRESHOLD TO DETECT SURGE ARRIVAL
      EPS_SURGE_ARR = MOTREA( ADRESS(2, 99) )
!
! LOGICAL KEYWORDS:
!
...
!     WHEN COMPUTING MAXZ, TMXZ, MAXV, TMXV, MAXH, MAXQ, TMXQ OR TSAR
...
!     FROM V9P0, IF A PREVIOUS COMPUTATION FILE IS DEFINED,
!     COMPUTATION CONTINUED IS AUTOMATICALLY ACTIVATED
      IF(T2D_FILES(T2DPRE)%NAME(1:1).NE.' ') THEN
        DEBU = .FALSE.
      ELSE
        DEBU = .TRUE.
      ENDIF
...
!     FROM V9P0, IF A REFERENCE FILE IS DEFINED,
...
!     FROM V9P0, IF A RESULT DELWAQ FILE IS DEFINED,
!     E.G. VELOCITY, DIFFUSIVITY, TEMPERATURE OR SALINITY
!     THE CORRESPONDING LOGICAL FOR DELWAQ IS AUTOMATICALLY ACTIVATED
!     E.G. VELO_DEL, DIFF_DEL, TEMP_DEL OR SALI_DEL (RESP.)
      IF(T2D_FILES(T2DDL4)%NAME(1:1).NE.' ') THEN
        SALI_DEL = .TRUE.
      ELSE
        SALI_DEL = .FALSE.
      ENDIF
      IF(T2D_FILES(T2DDL8)%NAME(1:1).NE.' ') THEN
        TEMP_DEL = .TRUE.
      ELSE
        TEMP_DEL = .FALSE.
      ENDIF
      IF(T2D_FILES(T2DDL9)%NAME(1:1).NE.' ') THEN
        VELO_DEL = .TRUE.
      ELSE
        VELO_DEL = .FALSE.
      ENDIF
      IF(T2D_FILES(T2DL10)%NAME(1:1).NE.' ') THEN
        DIFF_DEL = .TRUE.
      ELSE
        DIFF_DEL = .FALSE.
      ENDIF
...
!     OIL SPILL MODEL IS AUTOMATICALLY ACTIVATED
      IF(T2D_FILES(T2DMIG)%NAME(1:1).NE.' ') THEN
        SPILL_MODEL = .TRUE.
      ELSE
        SPILL_MODEL = .FALSE.
      ENDIF
...
!     FILE OF SETTINGS FOR THE OBSTACLES
      T2D_FILES(T2DOBS)%NAME=MOTCAR( ADRESS(4,110) )
!     RESULT FILE FOR THE OBSTACLES
      T2D_FILES(T2DOUT)%NAME=MOTCAR( ADRESS(4,111) )
!     FILE OF GEOMETRIES FOR THE OBSTACLES
      T2D_FILES(T2DOGE)%NAME=MOTCAR( ADRESS(4,112) )
!     TEMPORAL CONTROL FILE FOR THE OBSTACLES
      T2D_FILES(T2DOTS)%NAME=MOTCAR( ADRESS(4,113) )
!     FILE OF VARIABLE SETTINGS FOR THE OBSTACLES
      T2D_FILES(T2DOHY)%NAME=MOTCAR( ADRESS(4,114) )
!     WEIRS DATA FILE
...
!     FROM V9P0, IF A BREACHES DATA FILE IS DEFINED,
!     BREACH IS AUTOMATICALLY ACTIVATED
      IF(T2D_FILES(T2DBRC)%NAME(1:1).NE.' ') THEN
        BRECHE = .TRUE.
      ELSE
        BRECHE = .FALSE.
      ENDIF
...
!     FROM V9P0, IF A FLUXLINE FILE IS DEFINED,
!     CONTROL SECTIONS WITH FLUXLINE IS AUTOMATICALLY ACTIVATED
      IF(T2D_FILES(T2DFLX)%NAME(1:1).NE.' ') THEN
        DOFLUX = .TRUE.
      ELSE
        DOFLUX = .FALSE.
      ENDIF
...
!     FROM V9P0, IF A RESTART FILE IS DEFINED,
!     RESTART MODE IS AUTOMATICALLY ACTIVATED
      IF(T2D_FILES(T2DRST)%NAME(1:1).NE.' ') THEN
        RESTART_MODE = .TRUE.
      ELSE
        RESTART_MODE = .FALSE.
      ENDIF
...
!     SPALART ALLMARAS
      IF(ITURB.EQ.6) THEN
        SOREST(40)=.TRUE.
      ENDIF
!     HIGHEST WATER DEPTH AND ASSOCIATED TIME
      IF(SORLEO(43)) THEN
!       MAXH
        SOREST(43)=.TRUE.
!       TMXZ
        IF(SORLEO(28)) SOREST(28)=.TRUE.
      ENDIF
!     HIGHEST FLOWRATE AND ASSOCIATED TIME
      IF(SORLEO(44)) THEN
!       MAXQ
        SOREST(44)=.TRUE.
!       TMXQ
        IF(SORLEO(45)) SOREST(45)=.TRUE.
      ENDIF
!     TIME OF SURGE ARRIVAL + INITIAL WATER DEPTH
      IF(SORLEO(46)) THEN
!       TSAR
        SOREST(46)=.TRUE.
!       H0
        SOREST(47)=.TRUE.
!       IF TIME OF SURGE ARRIVAL HAS BEEN FOUND
        SOREST(48)=.TRUE.
      ELSEIF(SORLEO(47)) THEN
!       H0
        SOREST(47)=.TRUE.
      ENDIF
...
!-----------------------------------------------------------------------
!
!     USER MASK FOR OBSTACLES
!
      MSKOBS = (T2D_FILES(T2DOBS)%NAME(1:1).NE.' ').AND.
     &         (T2D_FILES(T2DOGE)%NAME(1:1).NE.' ')
!
!     MAKING SURE THAT OPTSOU = 2 FOR OBSTACLES WITH FINITE VOLUMES
!
      IF(EQUA(1:15).EQ.'SAINT-VENANT VF'.AND.OPTSOU.NE.2) THEN
        WRITE(LU,1020)
1020    FORMAT(1X,'TYPE OF SOURCES SWITCHED TO 2 WITH FINITE VOLUMES')
        OPTSOU = 2
      ENDIF
!
!     SORTING OUT SCHEME SPECIFICS FOR TRACER WITH OBSTACLES
!
      ALLOCATE(OPTOBS(MAX(1,NTRAC)))
!
      OPTOBS = OPTSOU
!
!     THESE SCHEMES BEHAVE DIFFERENTLY
      IF(EQUA(1:15).EQ.'SAINT-VENANT VF') THEN
        OPTOBS = -OPTOBS
      ELSE
        IF( NTRAC.GT.0 ) THEN
          DO I = 1,NTRAC
            IF( ICONVFT(I).EQ.13 ) OPTOBS(I) = OPTOBS(I) * 3
            IF( ICONVFT(I).EQ.14 ) OPTOBS(I) = OPTOBS(I) * 3
            IF( ICONVFT(I).EQ.15 ) OPTOBS(I) = OPTOBS(I) * 3
          ENDDO
        ENDIF
      ENDIF
!
!     MAKING SURE THAT IMPLICITATION TETAT = 0. FOR OBSTACLES
!
      IF(MSKOBS.AND.TETAT.GT.0.D0) THEN
        WRITE(LU,1030)
1030    FORMAT(1X,'IMPLICITATION FOR TRACERS SET TO 0. WITH OBSTACLES')
        TETAT = 0.D0
      ENDIF
!
...
!
      ...
      MSK = .FALSE.
      IF(OPTBAN.EQ.2.OR.MSKUSE.OR.MSKOBS) MSK = .TRUE.
      IF(EQUA(1:15).EQ.'SAINT-VENANT VF') THEN
...
!-----------------------------------------------------------------------
!
      IF((OPT_HNEG.EQ.2.OR.OPT_HNEG.EQ.3).AND.EQUA(14:15).EQ.'EF') THEN
        IF(ABS(AGGLOC-1.D0).GT.0.01D0.OR..NOT.CORCON.OR.OPTSUP(2).NE.0)
     &  THEN
          WRITE(LU,*) 'TREATMENT OF NEGATIVE DEPTHS =', OPT_HNEG
        ENDIF
        IF(ABS(AGGLOC-1.D0).GT.0.01D0) THEN
          AGGLOC = 1.D0
          WRITE(LU,*) 'MASS-LUMPING ON H AUTOMATICALLY SET TO 1.D0'
        ENDIF
        IF(.NOT.CORCON) THEN
          CORCON = .TRUE.
          WRITE(LU,*) 'CONTINUITY CORRECTION AUTOMATICALLY SET TO YES'
        ENDIF
        IF(OPTSUP(2).NE.0) THEN
          OPTSUP(2) = 0
          WRITE(LU,*) 'SUPG OPTION AUTOMATICALLY SET TO 0 FOR H'
        ENDIF
        IF(ABS(AGGLOC-1.D0).GT.0.01D0.OR..NOT.CORCON.OR.OPTSUP(2).NE.0)
     &  THEN
          WRITE(LU,*)
...
75          FORMAT(1X,'TYPE OF ADVECTION FOR TRACERS: ',I3,/,1X,
     ...
     &       ICONVFT(ITRAC).EQ.ADV_PSI_TF     ) THEN
            IF(TETAT.NE.0.D0) THEN
              TETAT = 0.D0
              WRITE(LU,6001) ITRAC
6001          FORMAT(1X,'LECDON: DISTRIBUTIVE SCHEME ON THE TRACER ',
     &        1I6,/,1X,'IMPOSED VALUE: IMPLICITATION = 0.')
            ENDIF
            IF(OPTSUP(2).NE.0) THEN
              OPTSUP(2) = 0
              WRITE(LU,8001) ITRAC
8001          FORMAT(1X,'LECDON: DISTRIBUTIVE SCHEME ON THE TRACER ',
...
!-----------------------------------------------------------------------
!
!     ONLY ALLOWS ASCII DROGUES FILE, NOT BINARY DROGUES FILE
!     WITH OIL SPILL MODEL
!
      IF(SPILL_MODEL) THEN
        IF(T2D_FILES(T2DBLO)%NAME(1:1).NE.' ') THEN
          IF(T2D_FILES(T2DFLO)%NAME(1:1).EQ.' ') THEN
            WRITE(LU,*) ' '
            WRITE(LU,*) 'WITH THE OIL SPILL MODEL, ONLY WRITING RESULTS'
            WRITE(LU,*) 'IN THE ASCII DROGUES FILE IS AVAILABLE,'
            WRITE(LU,*) 'NOT IN THE BINARY DROGUES FILE'
            WRITE(LU,*) ' '
            CALL PLANTE(1)
            STOP
!         BOTH BINARY DROGUES FILE AND ASCII DROGUES FILE ARE WRITTEN
!         IN THE STEERING FILE
          ELSE
            WRITE(LU,*) ' '
            WRITE(LU,*) 'WITH THE OIL SPILL MODEL, ONLY WRITING RESULTS'
            WRITE(LU,*) 'IN THE ASCII DROGUES FILE IS DONE,'
            WRITE(LU,*) 'NOT IN THE BINARY DROGUES FILE'
            WRITE(LU,*) ' '
          ENDIF
        ELSEIF(T2D_FILES(T2DFLO)%NAME(1:1).EQ.' ') THEN
          WRITE(LU,*) ' '
          WRITE(LU,*) 'FOR THE OIL SPILL MODEL, PLEASE GIVE AN'
          WRITE(LU,*) 'ASCII DROGUES FILE TO WRITE RESULTS'
          WRITE(LU,*) ' '
          CALL PLANTE(1)
          STOP
        ENDIF
...
!
!     FINITE VOLUME CORIOLIS
!
      IMPCOR = MOTINT(ADRESS(1,123))
!
...
!
!-----------------------------------------------------------------------
!     IF COUPLING WITH TOMAWAC WE CAN NOT HAVE VARIABLE TIME-STEP
      IF(DTVARI.AND.INCLUS(COUPLING,'TOMAWAC')) THEN
        WRITE(LU,*) 'VARIABLE TIME-STEP IS NOT ALLOWED '//
     &              'WITH COUPLING WITH TOMAWAC'
        CALL PLANTE(1)
        STOP
      ENDIF
!-----------------------------------------------------------------------
